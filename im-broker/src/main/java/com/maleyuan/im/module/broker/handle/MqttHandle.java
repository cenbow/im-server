...
public class MqttHandle {
	...
	void connect(Channel channel, MqttConnectMessage msg) {
		...
		MqttConnAckVariableHeader header = new MqttConnAckVariableHeader(MqttConnectReturnCode.CONNECTION_ACCEPTED, true);
	...
	void publish(Channel channel, MqttPublishMessage msg) {
		...
		MqttPublishVariableHeader pvhead = new MqttPublishVariableHeader("chat", msg.variableHeader().packetId() + 10);
		...
		MqttPubAckMessage mqttPubAckMessage = new MqttPubAckMessage(fixedHeader, MqttMessageIdVariableHeader.from(msg.variableHeader().packetId() + 10));
